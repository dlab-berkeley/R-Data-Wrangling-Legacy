...
This is wide data that we would like to transform to long data, but here if we do a usual `pivot_longer()` we will end up with the problem that we want to avoid in this section. 
...
Here the dates and the results are merged into one value. However, in situations where we have a common naming structure, we can use some of the built-in arguments of `pivot_longer()`.
...
For the challenges, this should combine some of the skills that we did in Part1, probably a select to drop down the number of uses and then a join of some kind. The first challenge could also require the by column to show different column names can be joined appropriately. 

The second challenge should be the creation of a country-year dataset. The first data frame can be a country year expand.grid() and then the second can be the useful information. 

## Storing a single unit in multiple tables. 

This is unquestionably going to be the read in multiple files stuff. 
