...
All messy data is unhappy in its own ways. To put some structure on data, we prefer that our data is *tidy*. In tidy data, every column is a variable, every row is an observation, and every cell is a single value (Wickham 2014). 
...
Consider the data below. Both data sets display information on heart rate observed in individuals across 3 different time periods. But the data are organized differently in each table.
...
Alternatively, we can load individual packages from the tidyverse by themselves. For example, we can load the `dplyr` package. 
...
Here is an example workflow putting together every single one of these functions along with a step to read in a dataset. The descriptive question that this workflow answers is "What is the average life expectancy between farmers and non-farmers in West Coast states?"
...
## We can also save the output of our workflow to a new object
wc_lifeExp <- jobs %>% 
  select(State, Occuptation, Age)%>%
  filter(State %in% c("Alaska","California", "Oregon", "Washington"))%>%
  mutate(in_ag = if_else(Occupation == "Farmer", "Yes", "No"))%>%
...
## is equivalent to 
select(jobs, State, Occupation, Age)
```
