...
jobs <- read_csv(here("data/jobs.csv"), show_col_types = F)
...
gapminder %>% 
  ...
  select(year, continent, continent_pop)
```
...
## Take my original data 
## I need to move it to longer 
## I'll need to break apart the columns R dplyr into google.com
## I'll need to make sure that every variable is tidy
```
